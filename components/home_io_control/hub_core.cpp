...
#endif
ESP_LOGCONFIG(detail::TAG, "========== ABOUT TO CALL radio_->init() ==========");
ESP_LOGCONFIG(detail::TAG, "radio_ pointer = %p", this->radio_);
  if (!this->radio_->init()) {
    delete this->radio_;
    this->radio_ = nullptr;
#ifdef IOHOME_LR1121_FIRMWARE_UPDATE
...
void IOHomeControlComponent::dump_config() {
  ...
  if (this->fem_en_pin_ != nullptr)
    LOG_PIN("  fem_en_pin_ Pin: ", this->fem_en_pin_);
